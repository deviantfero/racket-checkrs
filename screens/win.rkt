#lang racket/gui

(require "../libs/pdata.rkt")
(require "../frame.rkt")
(require "../ARCHIVOS/matriz.rkt")

(provide win-canvas%)

(define bg-win (make-object bitmap% "img/BOARD_WINNER.jpg"))

(struct gpt (x y))
(struct gbutton (pt-1 pt-2 img img-pt))
 

;--win canvas begins here
(define win-canvas%
  (class canvas%
    (inherit refresh)

    (define sel-button 'none)
    (define sel-button-pt 'none)

    (define back (make-object bitmap% "img/BACK_HOVER.jpg"))
    (define close (make-object bitmap% "img/CLOSE_HOVER.jpg"))

    (define winner (readWinner))

    (define buttons
      (list
       (gbutton (gpt 315 329) (gpt 410 385) back (gpt 304 319))
       (gbutton (gpt 535 329) (gpt 630 385) close (gpt 522 319))))

    (define (handle-mouse-motion x y)
      (let loop ([bl buttons])
        (if (empty? bl)
           (refresh)
           (let* ([b (car bl)]
                 [pt-1 (gbutton-pt-1 b)]
                 [pt-1-x (gpt-x pt-1)]
                 [pt-1-y (gpt-y pt-1)]
                 [pt-2 (gbutton-pt-2 b)]
                 [pt-2-x (gpt-x pt-2)]
                 [pt-2-y (gpt-y pt-2)])
             (if (and (> x pt-1-x) (> y pt-1-y) (< x pt-2-x) (< y pt-2-y))
                 (begin
                   (set! sel-button (gbutton-img b))
                   (set! sel-button-pt (gbutton-img-pt b))
                   (refresh))
                 (begin
                   (set! sel-button 'none)
                   (set! sel-button-pt 'none)
                   (loop (cdr bl))))))))
    

    (define (action x y)
      (cond
        ([and (and (> x (gpt-x (gbutton-pt-1 (car buttons)))) 
                     (< x (gpt-x (gbutton-pt-2 (car buttons)))))
                      (and (> y (gpt-y (gbutton-pt-1 (car buttons))))
                            (< y (gpt-y (gbutton-pt-2 (car buttons)))))] (begin (ActualizarHScore winner) (send mainframe show #f) (system "racket ./main.rkt")))

        ([and (and (> x (gpt-x (gbutton-pt-1 (cadr buttons)))) 
                     (< x (gpt-x (gbutton-pt-2 (cadr buttons)))))
                      (and (> y (gpt-y (gbutton-pt-1 (cadr buttons))))
                            (< y (gpt-y (gbutton-pt-2 (cadr buttons)))))] (begin (ActualizarHScore winner) (send mainframe show #f)))

        (else (void))))

    (define/override (on-event evt)
      (let ([evt-type (send evt get-event-type)]
            [evt-x (send evt get-x)]
            [evt-y (send evt get-y)])
        (cond
          [(eq? evt-type 'motion)
           (handle-mouse-motion evt-x evt-y)]
          [(eq? evt-type 'left-up)
            (begin
              (action evt-x evt-y)
              (printf "Click % (~a, ~a)~n" evt-x evt-y))])))

    (define/private (paint-game self dc)
      (send dc draw-bitmap bg-win 0 0)
      (send dc draw-text winner 433 254)
      (send dc set-pen "blue" 1 'solid) (send dc set-brush "white" 'transparent) (cond
        [(not (eq? sel-button 'none))
         (let ([img-x (gpt-x sel-button-pt)]
               [img-y (gpt-y sel-button-pt)])
           (send dc draw-bitmap sel-button img-x img-y))]))

    (super-new (paint-callback (lambda (c dc) (paint-game c dc))))))
